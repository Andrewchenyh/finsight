...
from backend.evals.legacy_retrieval_metrics import (
    evaluate_retrieval_results,
    score_question_result,
)
from backend.evals.legacy_schemas import EvalDataset
...
def run_eval(dataset_path: Path, modes: list[RetrievalMode]) -> None:
    ...
    for mode in modes:
        ...
        print("=" * 100)
        print(f"Mode: {mode}")
        print(f"Hit rate@k: {run_result.hit_rate_at_k:.2f}")
        print(
            "Mean heuristic precision@k: "
            f"{run_result.mean_heuristic_precision_at_k:.2f}"
        )

        if run_result.mean_first_hit_rank is not None:
            print(f"Mean first-hit rank: {run_result.mean_first_hit_rank:.2f}")
        else:
            print("Mean first-hit rank: n/a")
        ...
        for question_result in run_result.question_results:
            ...
            print(
                ...
                f"heuristic_precision={question_result.heuristic_precision_at_k:.2f}"
            )
        print()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run FinSight's legacy manually labeled retrieval evaluation."
    )
    parser.add_argument(
        "--dataset",
        default="data/evals/legacy/msft_2023_manual_labels.json",
        help="Path to the legacy manually labeled eval dataset JSON.",
    )
    parser.add_argument(
        "--modes",
        nargs="+",
